const create_location = function() {
    ...
    const location_gameImage = create_element({ Tag: "img", Attribute: { src: Config.Images.Game.Title, }, Parent: location_gameWrap });
...
function location_data() {
    ...
    const overviewImages = Config.Images.Locations[location_index].Overview;
...
function trainer_populate(location_index) {
    ...
    const reward_src = reward_index ? Config.Images.Items[reward_index].Bag : trainer_data[index].Reward ? get_directory({FirstMatch: true, Exact: true, File: [trainer_data[index].Reward.Reward], Path: [Path.Currency.Icon]}) : "";
    ...
    const item_src = item_index ? Config.Images.Items[item_index].Bag : "";
    ...
    trainer_data[index].Pokemon && trainer_data[index].Pokemon.forEach(p => {
        ...
        if (Data.Pokemon[pokemon_index]) {
            ...
            const pokemonImage = create_element({ Tag: "img", Class: ["pokemon_image"], Attribute: { src: Config.Images.Pokemon[pokemon_index].Battle.Default.Front.PNG || Config.Images.Pokemon[pokemon_index].Battle.Default.Front.GIF, }, Parent: pokemonWrap });
