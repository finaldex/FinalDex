const create_nav = function() {
    ...
    nav_options.forEach((opt, index) => {
        ...
        const nav_input = create_element({ Tag: "input", Attribute: { type: "radio", name: "navigation", id: `nav-${index + 1}`, value: opt.id, ...(opt.id === "dex" && { checked: true }), }, Event: { change: () => { const show = document.querySelectorAll(`#${opt.id}`); const hide = document.querySelectorAll(nav_options.filter(option => option.id !== opt.id).map(option => `#${option.id}`).join(', ')); toggleVisibility(show, hide); svg_update(document.querySelector("#location .map > div > div")); } }, Parent: nav_lbl });
