let create_tool = function() {
	...
	toolSectionHeaderGameImage.src = get_directory({FirstMatch: true, File: ["Title"], Path: [path.Game.Title]})
	...
	function createType() {
		...
		for (let i = 0; i < 2; i++) {
			...
			for (let q = 0; q < config.Types.length; q++) {
				let val = config.Types[q];
				...
				typeSidebarTitleDropOption.setAttribute("name",config.Types[q]);
				typeSidebarTitleDropOptionImg.src = get_directory({FirstMatch: true, File: [val+" Symbol 1"], Path: [path.Type.Icon], Game: ["Sword"]})
				typeSidebarTitleDropOptionImg.title = config.Types[q];
				typeSidebarTitleDropOptionImg.setAttribute("onerror","this.style.display='none';");
				typeSidebarTitleDropOptionTxt.innerText = config.Types[q];
				...
				typeSidebarTitleDropOption.addEventListener("click",function() {
					...
					img.src = get_directory({FirstMatch: true, File: [val+" Symbol 1"], Path: [path.Type.Icon], Game: ["Sword"]})
		...
		for (let i = 0; i < defAtt.length; i++) {
			...
			if (i == 0) {
				...
				cont = "when used against an opposing Pokemon with following types:";
			}
			...
			for (let r = 0; r < tempTypes.length; r++) {
				...
				if (q == 0 || r == 0) {
					...
					if (val != "") {
						...
						txt.innerText = val;
						img.src = get_directory({FirstMatch: true, File: [val+" Symbol 2"], Path: [path.Type.Icon], Game: ["Sword"]})
	...
	function createDamageCalc() {
		...
		toolSectionContentDMGOptionsContentPok.setAttribute("name","pokemon");
...
;
        ...
        let toolSectionContentDMGImportOpts = ["Import Pokemon","Copy Data Strings"];
		...
		toolSectionContentDMGReset.addEventListener("click",function() {
			...
			let lock = confirm("The Pokemon's data will not be saved.\nDo you want to continue?");
			if (lock) {
				let divBase = document.querySelectorAll("#contain > div#tool div#dmg div[name='battle'] span[name] > div[name]");
				let pokBase = document.querySelectorAll("#contain > div#tool div#dmg ol[name='pokemon'] span[name] > ul[name]");			
		...
		for (let m = 0; m < finaldata["Moves"]["Overview"].length; m++) {
			if (finaldata["Moves"]["Overview"][m]["Active"] == "true") {
				if (finaldata["Moves"]["Group"][m]["Group"] != "Z-Move" && finaldata["Moves"]["Group"][m]["Group"] != "Max Move" && finaldata["Moves"]["Group"][m]["Group"] != "G-Max Move") {
					tempMoves.push(finaldata["Moves"]["Overview"][m][header.Move.Reference["Name"]])
				}
			}
		}
		...
		for (let m = 0; m < tempMoves.length; m++) {
			let moveCategory = returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],tempMoves[m]);
			...
			if (moveCategory != undefined && moveCategory != "Status") {
				...
				toolSectionContentDMGMenuCenterTopOption.style.background = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],tempMoves[m])+")";
			}
		}
		*/
		...
		if (toolSectionContentDMGMenuCenterSelect.querySelector(":scope option[value='Pound']") != undefined) {
			...
			toolSectionContentDMGMenuCenterSelect.parentElement.style.color = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],toolSectionContentDMGMenuCenterSelect.value)+")";
			toolSectionContentDMGMenuCenterBottomTypeImg.src = get_directory({FirstMatch: true, File: [returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],toolSectionContentDMGMenuCenterSelect.value)], Path: [path.Type.Icon]})
			toolSectionContentDMGMenuCenterBottomTypeText.innerText = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],toolSectionContentDMGMenuCenterSelect.value);
			toolSectionContentDMGMenuCenterBottomCategoryImg.src = get_directory({FirstMatch: true, File: [returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],toolSectionContentDMGMenuCenterSelect.value)], Path: [path.Type.Icon]})
			toolSectionContentDMGMenuCenterBottomCategoryText.innerText = returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],toolSectionContentDMGMenuCenterSelect.value);
		}
		...
		toolSectionContentDMGMenuCenterSelect.addEventListener("change",function() {
			let movd = formatMoveData(this.value); movd = undDel(movd,"");this.title = movd;this.parentElement.style.color = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],this.value)+")";
		});
...
/*
		for (let q = 0; q < battleVariation.length; q++) {
			if (get_applicable(battleVariation[q]["Game"])) {
		...
		toolSectionContentDMGOptionsBattlesSelect.setAttribute("pokemon",battleVariation[0]["Pokemon"])
...
function timerSelector() {
	if (document.querySelector("#timerselector1").checked) {
	...
	if (document.querySelector("#timerselector2").checked) {
...
function typeSwitch() {
	...
	if (types.length > 0) {
		...
		for (let q = 0; q < arrImmune.length; q++) {
			...
			wrap.setAttribute("name",val);
			txt.innerText = val;
			img.src = get_directory({FirstMatch: true, File: [val], Path: [path.Type.Icon]})
			img.title = val;
			img.setAttribute("onerror","this.style.display = 'none'");
			img.setAttribute("onload","this.previousElementSibling.style.display ='none'");
