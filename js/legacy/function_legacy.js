...
/*
function getPokemonMediaPath(userFile,userPath,gender,games) {
					...
					for (let q = 0; q < arr[key].length; q++) {
						...
						if (file.includes(".png") || file.includes(".gif")) {
							for (let t = 0; t < userFile.length; t++) {
								...
								if (isNaN(int)) {
									int = getPokemonInt(userFile[t])
								}
							
								let mediaName = []
							
								let val1 = finaldata["Pokemon"]["Path"][int]["Number"];
								let val2 = finaldata["Pokemon"]["Path"][int]["Text"];
								...
								for (g = 0; g < gender.length; g++) {
									...
									if (check) {
										...
										if (get_applicable(source,games)) {
											return key+"/"+file;
										}
									}
								}
...
function getEvolutionFamily(i) {
	...
	let Next = undefined;
	stage = finaldata["Pokemon"]["Evolution Stage"];
	specie = finaldata["Pokemon"]["Evolution Specie"];
	let result = [];
	result.push(finaldata["Pokemon"]["Overview"][i]["Pokemon"])

	for (let q = 0; q < 10; q++) {
		Previous = finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Previous"]];
		Next = finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Next"]];
		if (Previous != undefined && Previous != "None") {
			result.push(Previous);
			Previous = finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Previous"]];
		}
		if (Next != undefined && Next != "None") {
			if (finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Next"]].includes(",")) {
				for (let y = 0; y < finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Next"]].split(",").length; y++) {
					Next = finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Next"]].split(",")[y];
					result.push(Next);
				}
			} else {
				Next = finaldata["Pokemon"]["Evolution Specie"][i][header.Pokemon.EvolutionSpecie["Next"]];
		...
		if (result[q] != undefined) {
			i = parseInt(getPokemonInt(result[q]));
		}
	}
	for (let q = 0; q < result.length; q++) {
		let obj = new Object();
		obj["Pokemon"] = result[q];
		obj["ID"] = getPokemonID(result[q]);
...
function getIntID(int, id) {
	...
	if (id == undefined || id == "") { // int --> id
		for (let i = 0; i < finaldata["Pokemon"]["Overview"].length; i++) {
			if (int == i) {
				return finaldata["Pokemon"]["Overview"][i]["ID"];
			}
		}
	} else if (int == undefined || int == "") { // id --> int
		for (let i = 0; i < finaldata["Pokemon"]["Overview"].length; i++) {
			if (id == finaldata["Pokemon"]["Overview"][i]["ID"]) {
				return i;
			}
		}
	}
...
function getMachineMove(machine) {
	...
	for (i = 0; i < arr.length; i++) {
		...
		if (arr[i][header.Move.Machine["Machine"]] == machine) {
			result = arr[i][header.Move.Reference["Name"]]
		}
	}
	return result;
}
...
function getMoveMachine(move) {
	...
	let result;
	for (i = 0; i < arr.length; i++) {
		if (arr[i][header.Move.Reference["Name"]] == move) {
			result = arr[i][header.Move.Machine["Machine"]]
		}
	}
	return result;
}
...
function Continuation(arr, column, style) {
	...
	if (style == "Single") {
		...
		for (i = 0; i < finaldata["Game"]["Overview"].length; i++) {
			if (parseInt(finaldata["Game"]["Overview"][i]["ID"]) <= config.ID) {
				names.push(finaldata["Game"]["Overview"][i]["Name"]);
			}
		}
		for (i = 0; i < finaldata["Game"]["Overview"].length; i++) {
			if (finaldata["Game"]["Overview"][i]["Name"] == GameName) {
				if (finaldata["Game"]["Overview"][i]["Sibling"] != undefined) {
					if (finaldata["Game"]["Overview"][i]["Sibling"].includes(",")) {
						for (q = 0; q < finaldata["Game"]["Overview"][i]["Sibling"].split(",").length; q++) {
							for (u = 0; u < finaldata["Game"]["Overview"].length; u++) {
								if (finaldata["Game"]["Overview"][u]["ID"] == finaldata["Game"]["Overview"][i]["Sibling"].split(",")[q]) {
									names.push(finaldata["Game"]["Overview"][u]["Name"]);
								}
							}
						}
					} else {
						for (u = 0; u < finaldata["Game"]["Overview"].length; u++) {
							if (finaldata["Game"]["Overview"][u]["ID"] == finaldata["Game"]["Overview"][i]["Sibling"]) {
								names.push(finaldata["Game"]["Overview"][u]["Name"]);
							}
						}
					}
		...
		for (i = 0; i < temparr1.length; i++) {
			let boolean = [];
			if (temparrtype[i] == "-") {
				for (q = 0; q < finaldata["Game"]["Overview"].length; q++) {
					if (finaldata["Game"]["Overview"][q]["Name"] == temparr1[i][0]) {
						temprun = true;
					}
					if (finaldata["Game"]["Overview"][q]["Name"] == temparr1[i][1]) {
						tempres.push(finaldata["Game"]["Overview"][q]["Name"])
						temprun = false;
					}
					if (temprun) {
						tempres.push(finaldata["Game"]["Overview"][q]["Name"])
...
function getRegionalID(seperator,id,dex) {
	...
	let arr = finaldata["Pokemon"]["Pokedex ID"];
	let tempid;
    let result;
...
function getFullGameName(name) {
	...
	let arr = finaldata["Game"]["Overview"];
	for (let q = 0; q < arr.length; q++) {
		if (arr[q]["Name"] == name) {
...
function getPokemonID(name) {
	...
	let arr = finaldata["Pokemon"]["Overview"];
	for (let q = 0; q < arr.length; q++) {
		if (arr[q]["Pokemon"] == name) {
			return arr[q]["ID"];
		}
	}
}
...
function getEvolutionStage(name) {
	...
	let arr = finaldata["Pokemon"]["Evolution Stage"];
	for (let q = 0; q < arr.length; q++) {
		if (arr[q]["Pokemon"] == name) {
			return arr[q][header.Pokemon.EvolutionStage["Pokemon Stage"]];
		}
	}
}
...
function getPokemonInt(name) {

	let arr = finaldata["Pokemon"]["Form"];
	for (let q = 0; q < arr.length; q++) {
		if (arr[q]["Pokemon"] == name || arr[q][header.Pokemon.Form["Form"]] == name) {
			return q;
		}
	}
}
...
function returnSortedItemsList(i) {
	...
	let items = finaldata["Items"]["Overview"].map(el => el["Pocket"] == "Berries" ? {...el, ["Pocket"]: "a"} : el).map(el => el["Pocket"] == "Items" || el["Pocket"] == "Other Items"  ? {...el, ["Pocket"]: "b"} : el).map(el => el["Pocket"] != "a" && el["Pocket"] != "b" ? {...el, ["Pocket"]: "c"} : el);

	items = sortObjectArray(items,"Pocket",true);

	let result = [];
	
	if (finaldata["Pokemon"]["Form Item"][i] != undefined) {
...
function dataStringToObj(data) {
	...
	// Example String: "Pokemon:Arcanine|Level:5|Ability:Flash Fire|Nature:Quiet|Gender:♀|Friendship:8|Item:Bluk Berry|Moves:Aqua Ring,,,Amnesia|EVs:9,,,24,7,|IVs:,,31,,19,21"

	const instances = data.includes("\n") ? data.split("\n") : [data]
	...
	for (i = 0; i < instances.length; i++) {
		...
		const pairs = data.includes('|') ? data.split('|') : [data];

		let obj = {};
		
		// Define the order for EVs and IVs
		const statsOrder = ['HP', 'Atk', 'Def', 'SpAtk', 'SpDef', 'Speed'];
		...
		pairs.forEach(pair => {
			...
			switch (key) {
				case 'Moves':
					obj.Moves = value.split(','); // Split Moves into an array
					break;
				case 'EVs':
					obj.EVs = {};
					value.split(',').forEach((v, index) => {
						obj.EVs[statsOrder[index]] = v ? Number(v) : 0; // Assign values with defaults
					});
					break;
				case 'IVs':
					obj.IVs = {};
					value.split(',').forEach((v, index) => {
						obj.IVs[statsOrder[index]] = v ? Number(v) : 0; // Assign values with defaults
					});
					break;
				default:
					obj[key] = value; // For other key-value pairs
			}
		});
...
function getPokemonName(int,alt) {

	let arr = finaldata["Pokemon"]["Form"];
	if (alt == "Alt") {
		for (let i = 0; i < arr.length; i++) {
			if (int == i) {
				if (arr[i][header.Pokemon.Form] != undefined && !arr[i]["Variant"].includes("Default")) {
					return arr[i][header.Pokemon.Form];
				} else {
					return arr[i]["Pokemon"];
				}
			}
		}
	}
		...
		for (let i = 0; i < arr.length; i++) {
			if (int == i) {
				if (arr[i][header.Pokemon.Form] != undefined) {
					return arr[i][header.Pokemon.Form];
				} else {
					return arr[i]["Pokemon"];
				}
...
function getPokemonForm(i) {

    let id = getIntID(i,"");
    let result = [];

    for (let u = 0; u < finaldata["Pokemon"]["Form"].length; u++) {
		if (finaldata["Pokemon"]["Form"][u]["ID"] == id && finaldata["Pokemon"]["Overview"][u]["Active"] == "true") {
            if (finaldata["Pokemon"]["Form"][u][header.Pokemon.Form["Form"]] != undefined) {
                result.push(finaldata["Pokemon"]["Form"][u][header.Pokemon.Form["Form"]]);
            }
            else {
                result.push(finaldata["Pokemon"]["Form"][u]["Pokemon"]);
            }
		}
	}
...
function getPositionAbility(i,column) {
	...
	let arr = finaldata["Pokemon"]["Ability"];
	let keys1 = Object.keys(header.Pokemon.Ability);
	let keys2 = header.Pokemon.Ability;
...
function get_applicable_old(val) {
...
function getItemIcon(item) {
	...
	let arr = finaldata["Items"]["Overview"];
...
function getEvolutionData(i,column) {
	...
	let arrName = finaldata["Pokemon"]["Evolution Specie"];
	let arrStage = finaldata["Pokemon"]["Evolution Stage"];
	let arrMethod = finaldata["Pokemon"]["Evolution Method"];

	let result = [];
	let pokemon = arrName[i][header.Pokemon.EvolutionSpecie[column]];

	if (pokemon == undefined) {
		i = getDefaultInt(i);
		pokemon = arrName[i][header.Pokemon.EvolutionSpecie[column]];
	}


	if (pokemon != undefined && pokemon != "None") {
		if (pokemon.includes(",")) {
			pokemon = pokemon.split(",");
			for (q = 0; q < pokemon.length; q++) {
				let stage = arrStage[getPokemonInt(pokemon[q])][header.Pokemon.EvolutionStage["Pokemon Stage"]];
				let method = arrMethod[getPokemonInt(pokemon[q])][header.Pokemon.EvolutionMethod["Type"]];
				let factor = arrMethod[getPokemonInt(pokemon[q])][header.Pokemon.EvolutionMethod["Factor"]];
				let additional = arrMethod[getPokemonInt(pokemon[q])][header.Pokemon.EvolutionMethod["Additional"]];
				let gender = arrMethod[getPokemonInt(pokemon[q])][header.Pokemon.EvolutionMethod["Gender"]];
		
				if (pokemon[q] == undefined) {
					pokemon[q] = arrName[getDefaultInt(i)][header.Pokemon.EvolutionSpecie[column]];
				}
				if (stage == undefined) {
					stage = arrStage[getDefaultInt(i)][header.Pokemon.EvolutionStage["Pokemon Stage"]];
				}

				if (method == undefined && factor == undefined && additional == undefined && gender == undefined) {
					method = arrMethod[getDefaultInt(i)][header.Pokemon.EvolutionMethod["Type"]];	
					factor = arrMethod[getDefaultInt(i)][header.Pokemon.EvolutionMethod["Factor"]];
					additional = arrMethod[getDefaultInt(i)][header.Pokemon.EvolutionMethod["Additional"]];
					gender = arrMethod[getDefaultInt(i)][header.Pokemon.EvolutionMethod["Gender"]];
				}

				if (pokemon != undefined) {
					let obj = new Object();
					obj["Integer"] = getPokemonInt(pokemon[q]);
					obj["Pokemon"] = pokemon[q];
					obj["Stage"] = stage;
					obj["Method"] = method;
					obj["Factor"] = factor;
			...
			let stage = arrStage[getPokemonInt(pokemon)][header.Pokemon.EvolutionStage["Pokemon Stage"]];
			let method = arrMethod[getPokemonInt(pokemon)][header.Pokemon.EvolutionMethod["Type"]];
			let factor = arrMethod[getPokemonInt(pokemon)][header.Pokemon.EvolutionMethod["Factor"]];
			let additional = arrMethod[getPokemonInt(pokemon)][header.Pokemon.EvolutionMethod["Additional"]];
			let gender = arrMethod[getPokemonInt(pokemon)][header.Pokemon.EvolutionMethod["Gender"]];
	
			if (pokemon == undefined) {
				pokemon = arrName[getDefaultInt(i)][header.Pokemon.EvolutionSpecie[column]];
			}
			if (stage == undefined) {
				stage = arrStage[getDefaultInt(i)][header.Pokemon.EvolutionStage["Pokemon Stage"]];
			}
			...
			if (pokemon != undefined) {
				let obj = new Object();
				obj["Integer"] = getPokemonInt(pokemon);
				obj["Pokemon"] = pokemon;
				obj["Stage"] = stage;
				obj["Method"] = method;
				obj["Factor"] = factor;
...
function getOffspringData(i) {
	...
	let arr = finaldata["Pokemon"]["Offspring"];
	let result = [];

	let pok = getPokemonName(i);

	for (q = 0; q < arr.length; q++) {
		if (arr[q][header.Pokemon.Offspring["Offspring"]] != undefined) {
			if (arr[q][header.Pokemon.Offspring["Offspring"]].includes(",")) {
				let tempArr1 = arr[q][header.Pokemon.Offspring["Offspring"]].split(",");
	
				for (u = 0; u < tempArr1.length; u++) {
					if (tempArr1[u] == pok) {
						let obj = new Object();
						obj["Integer"] = q;
						obj["Pokemon"] = getPokemonName(q);
						if (arr[q][header.Pokemon.Offspring["Factor"]] != undefined) {
							obj["Factor"] = arr[q][header.Pokemon.Offspring["Factor"]].split(",")[u];
						}
						result.push(obj)
					}
				}
				...
				if (arr[q][header.Pokemon.Offspring["Offspring"]] == pok) {
					let obj = new Object();
					obj["Integer"] = q;
					obj["Pokemon"] = getPokemonName(q);
					if (arr[q][header.Pokemon.Offspring["Factor"]] != undefined) {
						obj["Factor"] = arr[q][header.Pokemon.Offspring["Factor"]];
					}
					result.push(obj)
				}
			}
...
function formatEvoBreedText(i,type) {
	...
	if (type == "Breed") {
		...
		for (let q = 0; q < finaldata["Pokemon"]["Offspring"].length; q++) {
			if (finaldata["Pokemon"]["Overview"][i]["Active"] == "true") {
				if (finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Offspring"]] != undefined) {
					if (finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Offspring"]].includes(",")) {
						let ofs = finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Offspring"]].split(",");
						for (let u = 0; u < ofs.length; u++) {
							if (getPokemonInt(ofs[u]) == i) {
								if (q != i) {
									let obj = new Object();
									obj["Integer"] = q;
									obj["Pokemon"] = getPokemonName(q);
									...
									if (finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Factor"]] != undefined) {
										if (finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Factor"]].split(",").includes(",")) {
											obj["Offspring Factor"] = finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Factor"]].split(",")[u];
										}
										else {
											obj["Offspring Factor"] = finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Factor"]];
										}
									}
									else {
						...
						if (getPokemonInt(finaldata["Pokemon"]["Offspring"][q][header.Pokemon.Offspring["Offspring"]]) == i) {
							if (q != i) {
								let obj = new Object();
								obj["Integer"] = q;
								obj["Pokemon"] = getPokemonName(q);
		...
		for (let q = 0; q < res.length; q++) {
			...
			for (let u = 0; u < pks.length; u++) {
				let att = "type='invert' onclick='modalData()' value='"+getPokemonInt(pks[u])+"'";
				pks[u] = "<b "+att+">"+pks[u]+"</b>";
			}
...
function returnMoveLearnset(move,conditions) {
	...
	let arr1 = finaldata["Pokemon Learnset"]["Level Up"];
	let arr2 = finaldata["Pokemon Learnset"]["Evolution"];
	let arr3 = finaldata["Pokemon Learnset"]["Machine"];
	let arr4 = finaldata["Pokemon Learnset"]["Breeding"];

	let result = [];

	for (let i = 0; i < arr1.length; i++) {
		if (get_applicable(arr1[i]["Game"])) {
			if (arr1[i]["Move"] == move) {
				result.push(arr1[i]["Pokemon"]);
			}
		}
	}
	for (let i = 0; i < arr2.length; i++) {
		if (get_applicable(arr2[i]["Game"])) {
			if (arr2[i]["Move"] == move) {
				result.push(arr2[i]["Pokemon"]);
			}
		}
	}
	for (let i = 0; i < arr3.length; i++) {
		if (get_applicable(arr3[i]["Game"])) {
			if (arr3[i]["Move"] == move) {
				result.push(arr3[i]["Pokemon"]);
			}
		}
	}
	for (let i = 0; i < arr4.length; i++) {
		if (get_applicable(arr4[i]["Game"])) {
			if (arr4[i]["Move"] == move) {
				result.push(arr4[i]["Pokemon"]);
			}
		}
	}
...
function referenceLink(text) {
	...
	let items = finaldata["Items"]["Overview"];
	let abilities = finaldata["Abilities"]["Overview"];
	let moves = finaldata["Moves"]["Overview"];
	let poks = finaldata["Pokemon"]["Overview"];
	...
	for (let i = 0; i < abilities.length; i++) {
		if (get_applicable(abilities[i]["Game"])) {
	...
	for (let i = 0; i < moves.length; i++) {
		if (moves[i]["Active"]) {
			let move = moves[i][header.Move.Reference["Name"]];
			if (move.includes(" ")) {
				moveArr.push(move)
			}
		}
...
function getGames(option) {
	let ref = finaldata["Game"]["Overview"];
	let result = [];

	let g = ref[config.ID-1];
