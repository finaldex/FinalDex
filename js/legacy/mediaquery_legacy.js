...
let datas = ["Directory","Game Dataset","Locations Dataset","Pokemon Dataset","Items Dataset"]
...
document.querySelector("nav span[name='set'] button[name='pokemon']").addEventListener("click",setAll);
...
function setAll() {
    ...
    if (tar.getAttribute("name") == "pokemon") {
        for (let i = 0; i < finaldata["Pokemon"]["Path"].length; i++) {
            let vals = []
            if (finaldata["Pokemon"]["Path"][i]["Number"] != undefined) {
                vals.push(finaldata["Pokemon"]["Path"][i]["Number"])
            }
            if (finaldata["Pokemon"]["Path"][i]["Text"] != undefined) {
                vals.push(finaldata["Pokemon"]["Path"][i]["Text"])
            }
            result.push(vals.join("-"));
        }
    }
    else if (tar.getAttribute("name") == "items") {
        for (let i = 0; i < finaldata["Items"]["Overview"].length; i++) {
            if (get_applicable(finaldata["Items"]["Overview"][i]["Game"])) {
                if (finaldata["Items"]["Overview"][i]["Use"] == "true") {
                    if (finaldata["Items"]["Overview"][i]["Item"] != undefined) {
                        result.push(finaldata["Items"]["Overview"][i]["Item"]);
                    }
                }
            }
        }
        ...
        for (let i = 0; i < finaldata["Locations"]["Overview"].length; i++) {
            if (get_applicable(finaldata["Locations"]["Overview"][i]["Game"])) {
                if (finaldata["Locations"]["Overview"][i]["Location"] != undefined) {
                    result.push(finaldata["Locations"]["Overview"][i]["Location"]);
                }
            }
        }
