function DMGCalcStart() {
    ...
    let userPokBase = document.querySelector("#contain > div#tool div#dmg ol[name='pokemon'] span[name='"+userTeam+"'] > ul[name='"+userID+"']");
    ...
    let battleSizes = battleSelect.getAttribute("pokemon");
	...
	// User Pokemon
	let userPokemonPath = userPokBase.querySelector(":scope *[name='pokemon'] select");
	...
	// Move
	let movePower = returnArrValue(finaldata["Moves"]["Power"],header.Move.Reference["Name"],header.Move.Power["Power"],movePath.value);
	let moveAccuracy = returnArrValue(finaldata["Moves"]["Accuracy"],header.Move.Reference["Name"],header.Move.Accuracy["Accuracy"],movePath.value);
	let moveCategory = returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],movePath.value);
	let moveType = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],movePath.value);
	let movePriority = returnArrValue(finaldata["Moves"]["Priority"],header.Move.Reference["Name"],header.Move.Priority["Priority"],movePath.value);
	let moveGroup = returnArrValue(finaldata["Moves"]["Group"],header.Move.Reference["Name"],"Group",movePath.value);
	let moveRange = returnArrValue(finaldata["Moves"]["Range"],header.Move.Reference["Name"],"Range",movePath.value);
	...
	if (check) {
		for (let i = 0; i < allDivBase.length; i++) {
			if (allDivBase[i].getAttribute("data-string") != "" && !allDivBase[i].getAttribute("data-string").includes("pok:|")) {
                ...
                let tarPokBase = document.querySelector("#contain > div#tool div#dmg ol[name='pokemon'] span[name='"+tarTeam+"'] > ul[name='"+tarID+"']");
                ...
                // Target Pokemon
				let tarPokemonPath = tarPokBase.querySelector(":scope *[name='pokemon'] select");
				...
				if ("Type" && "Category") {
					...
					for (let u = 0; u < finaldata["Moves"]["Additional"].length; u++) {
						if (get_applicable(finaldata["Moves"]["Additional"][u]["Game"])) {
				...
				if (check) {
					...
					for (let h = 0; h < countInputPath.value; h++) {
						...
						if ("Factors Pre") {
								...
								if (ZMovePath != undefined && ZMovePath.checked) {
									...
									for (let r = 0; r < finaldata["Moves"]["Call"].length; r++) {
										if (finaldata["Moves"]["Call"][r]["Call"] == movePath.value) {
											if (finaldata["Moves"]["Call"][r]["Type"] == "Z-Move") {
												if (finaldata["Moves"]["Call"][r]["Pokemon"] != undefined) {
													let vals = splitStr(finaldata["Moves"]["Call"][r]["Pokemon"],",");
													for (let t = 0; t < vals.length; t++) {
														if (vals[t] == userPokemonPath.value) {
															check1 = true;
														}
													}
												}
									...
									if (check1 && check2) {
										...
										for (let u = 0; u < finaldata["Moves"]["Type"].length; u++) {
											if (finaldata["Moves"]["Type"][u][header.Move.Reference["Name"]].includes("(")) {
												if (returnArrValue(finaldata["Moves"]["Group"],header.Move.Reference["Name"],"Group",finaldata["Moves"]["Type"][u][header.Move.Reference["Name"]]) == "Z-Move") {
													if (finaldata["Moves"]["Type"][u][header.Move.Type["Type"]] == moveType) {
														if (finaldata["Moves"]["Type"][u][header.Move.Reference["Name"]].includes(moveCategory)) {
															movePath.parentElement.setAttribute("data-special",finaldata["Moves"]["Type"][u][header.Move.Reference["Name"]].replaceAll(" ("+moveCategory+")",""));
															break;
														}
													}
												}
							...
							if ("Power") {
								if (movePath.value == "Hidden Power") {
									...
									let val = getIntData(getPokemonInt(userPokemonPath.value),finaldata["Pokemon"]["Weight"],"Metric Values_1-8");
									val = undDel(val,0);
								...
								else if (movePath.value == "Triple Kick" && config.Generation >= 3) {
									Power = Power*(h+1);
								}
									...
									let val1 = getIntData(getPokemonInt(userPokemonPath.value),finaldata["Pokemon"]["Weight"],"Metric Values_1-8");
									let val2 = getIntData(getPokemonInt(tarPokemonPath.value),finaldata["Pokemon"]["Weight"],"Metric Values_1-8");
									val1 = undDel(val1,0);
									val2 = undDel(val2,0);
									...
									if (userItemPath != undefined) {
										...
										if (userItemPath.value.includes("TR")) {
											let val = returnArrValue(finaldata["Moves"]["Power"],header.Move.Reference["Name"],header.Move.Power["Power"],getMachineMove(userItemPath.value));
							...
							for (let u = 0; u < finaldata["Items"]["Type Enhance"].length; u++) {
								if (userItemPath != undefined && finaldata["Items"]["Type Enhance"][u]["Item"] == userItemPath.value) {
									if (get_applicable(finaldata["Items"]["Type Enhance"][u]["Game"])) {
										if (finaldata["Items"]["Type Enhance"][u]["Type"] == moveType) {
											var check = false;
											if (finaldata["Items"]["Type Enhance"][u]["Pokemon"] == undefined) {
												check = true;
											}
											else if (finaldata["Items"]["Type Enhance"][u]["Pokemon"].includes(",")) {
												check 
												let tempPok = finaldata["Items"]["Type Enhance"][u]["Pokemon"].split(",")
												for (let p = 0; p < tempPok.length; p++) {
													if (getPokemonName(getPokemonInt(tempPok[p])) == getPokemonName(getDefaultInt(getPokemonInt(userPokemonPath.value)))) {
														check = true;
														break;
													}
												}
											}
											else if (getPokemonName(getPokemonInt(finaldata["Items"]["Type Enhance"][u]["Pokemon"])) == getPokemonName(getDefaultInt(getPokemonInt(userPokemonPath.value)))) {
												check = true;
											}
											if (check) {
							...
							if (TerrainElectricPath != undefined && TerrainElectricPath.checked) {
								if (moveType == "Electric") {
									if (DMGCheckGrounded(tar)) {
										if (config.Generation >= 6 && config.Generation <= 7) {
											Power = Power*1.5;
										}
										else if (config.Generation >= 8) {
											Power = Power*1.3;
										}
									}
								}
						...
						if ("Factors Generation") {
							if (config.Generation == 1) { // DMG Generation 1
								if (moveCategory == "Physical") {
							...
							else if (config.Generation == 2) { // DMG Generation 2
							...
							else if (config.Generation == 3) { // DMG Generation 3
								...
								if (battleSize > 2) {
									if (tar.length > 1) {
										if (moveRange != "Affects all Pokemon adjacent to the user") {
											Targets = 0.5;
										}
									}
								}
							...
							else if (config.Generation == 4) { // DMG Generation 4
							...
							else if (config.Generation >= 5) { // DMG Generation 5+
								...
								*/
								...
								if (tarDynamaxPath != undefined) {
									if (tarDynamaxPath.checked || userPokemonPath.value.includes("Gigantamax")) {
								...
								if (tarAbilityPath.value == "Fluffy") {
									if (movePath.value != "Sunsteel Strike" && movePath.value != "Searing Sunraze Smash") {
										if (userAbilityPath.value != "Long Reach") {
											if (returnArrValue(finaldata["Moves"]["Other Moves"],header.Move.Reference["Name"],"Contact",movePath.value) == "Makes contact") {
												Fluffy1 = 0.5;
											}
										}
									}
								...
								if (tarAbilityPath.value == "Punk Rock") {
									if (returnArrValue(finaldata["Moves"]["Other Moves"],header.Move.Reference["Name"],"Sound-Based",movePath.value) == "Is a sound-tard move") {
										PunkRock = 0.5;
									}
								}
								...
								if (ProtectionPath.checked) {
									let grp = returnArrValue(finaldata["Moves"]["Group"],header.Move.Reference["Name"],"Group",movePath.value);
									...
									if (finaldata["Moves"]["Additional"][u]["Move"] == movePath.value) {
										...
										if (finaldata["Moves"]["Additional"][u]["Additional"] == "Sound") {
											if (tarAbilityPath != undefined && tarAbilityPath.value == "Soundproof") {
												var check = true;
												if (config.Generation == 5 && movePath.value == "Heal Bell") {
													check = false;
												}
												if (check) {
													Affected = false;
						...
						if ("Calculation") {
							...
							if (!Immune && Affected && moveAccuracy != 0) {
								...
								// Calculation Apply //
								if (movePath.value == "Triple Kick" && config.Generation == 2 || movePath.value == "Spit Up" || movePath.value == "Magnitude") { // Variable Power (Break Multiple Hits Loop)
						...
						if ("Info" && h == 0) {
							if (config.Generation == 1 || config.Generation == 4) {
								if (Type1*Type2 == 2) {
				...
				if ("End of turn") {
					...
					for (let u = 0; u < finaldata["Items"]["Restoration"].length; u++) {
						if (get_applicable(finaldata["Items"]["Restoration"][u]["Game"])) {
				...
				function DMGCalc_Accuracy() {
					...
					if (config.Generation >= 6) {
						...
						if (get_applicable("X,Y,Omega Ruby,Alpha Sapphire,Sun,Moon,Ultra Sun,Ultra Moon")) {
							let af = undDel(userAffectionPath.value,0);
							if (af >= 150) {
								Affection = 10;
							}
						}
						else if (get_applicable("Lets Go Pikachu,Lets Go Eevee")) {
							if (fr >= 200) {
								Affection = 10;
							}
						}
					...
					if (returnArrValue(finaldata["Moves"]["Accuracy"],header.Move.Reference["Name"],header.Move.Accuracy["Accuracy"],movePath.value) == undefined) {
						moveAccuracy = 100;
					}
				...
				function DMGCalc_Critical() {
					...
					for (let a = 0; a < finaldata["Moves"]["Additional"].length; a++) {
						if (finaldata["Moves"]["Additional"][a]["Additional"] == "High Critical-hit Ratio") {
							if (get_applicable(finaldata["Moves"]["Additional"][a]["Game"])) {
					...
					if (config.Generation == 1) {
						let tarspeed = parseInt(returnData(getPokemonInt(userPokemonPath.value),"Base Stats Speed","")[0]);
		...
		if ('Extra') {
			...
			moveTypeImgPath.src = get_directory({FirstMatch: true, File: [moveType], Path: [path.Type.Icon]})
			moveCategoryImgPath.src = get_directory({FirstMatch: true, File: [moveCategory], Path: [path.Move.Category]})
		...
		for (let i = 0; i < DMGCalculation.length; i++) {
			...
			for (let q = 0; q < DMGCalculation[i].length; q++) {
				...
				pokBase = document.querySelector("#contain > div#tool div#dmg ol[name='pokemon'] span[name='"+Team+"'] > ul[name='"+ID+"']");
...
function DMGFindCurrentHP(base) {
    ...
    let pokBase = document.querySelector("#contain > div#tool div#dmg ol[name='pokemon'] span[name='"+team+"'] > ul[name='"+id+"']");
...
function DMGCalcPokStats(base) {
	...
	// Pokemon
	let pokemonPath = pokBase.querySelector(":scope *[name='pokemon'] select");
	...
	if (pokemonPath.value != "") {
		let int = getPokemonInt(pokemonPath.value);
		...
		for (let i = 0; i < totalPath.length; i++) {
			...
			if (lvl != 0) {
				...
				if (paraPath.checked) {
					if (stat == "Speed") {
						let poktype = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],pokemon.value);
						var check = true;
						if (config.Generation >= 6) {
						...
						if (check) {
							if (config.Generation >= 1 && config.Generation <= 6) {
								let newVal = val*0.75;
								totalPath[i].setAttribute("min",newVal);
								totalPath[i].setAttribute("max",newVal);
								totalPath[i].value = newVal;
				...
				for (let b = 0; b < badgesPath.length; b++) {
					...
					if (input.checked) {
						if (get_applicable("Red,Blue,Yellow")) {
						...
						else if (get_applicable("Gold,Silver,Crystal")) {
						...
						else if (get_applicable("FireRed,LeafGreen")) {
							if (badgesPath[b].getAttribute("name") == "Boulder Badge") {
								if (totalPath[i].getAttribute("name") == "Attack") {
									totalPath[i].value = totalPath[i].value * 1.125;
								}
						...
						else if (get_applicable("Ruby,Sapphire,Emerald")) {
...
function DMGSetInfo() {
	...
	let userAbilityPath = userPokBase.querySelector(":scope *[name='ability'] select");
	let userPokPath = userPokBase.querySelector(":scope *[name='pokemon'] select");
	let userLevelPath = userPokBase.querySelector(":scope *[name='level'] input");
	let userItemPath = userPokBase.querySelector(":scope *[name='item'] select");
	...
	let moveType = undDel(returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],moveSelect.value),"None");
	let moveCategory = undDel(returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],moveSelect.value),"None");
	let movePower = undDel(returnArrValue(finaldata["Moves"]["Power"],header.Move.Reference["Name"],header.Move.Power["Power"],moveSelect.value),0);
	let moveAccuracy = undDel(returnArrValue(finaldata["Moves"]["Accuracy"],header.Move.Reference["Name"],header.Move.Accuracy["Accuracy"],moveSelect.value),"0%");
	let moveCritical = "0%";
	let moveGroup = undDel(returnArrValue(finaldata["Moves"]["Group"],header.Move.Reference["Name"],"Group",moveSelect.value),"None");
	let moveRange = undDel(returnArrValue(finaldata["Moves"]["Range"],header.Move.Reference["Name"],"Range",moveSelect.value),"None");
	...
	for (let l = 0; l < specLis.length; l++) {
		...
		/*
			...
			let uncallable = [];
			if (config.Generation == 4 || config.Generation == 5) {
			...
			if (moveSelect.value != "Struggle") {
				...
				for (let r = 0; r < finaldata["Moves"]["Call"].length; r++) {
					if (finaldata["Moves"]["Call"][r]["Call"] == moveSelect.value) {
						if (finaldata["Moves"]["Call"][r]["Type"] == "Z-Move") {
							if (finaldata["Moves"]["Call"][r]["Pokemon"] != undefined) {
								if (finaldata["Moves"]["Call"][r]["Pokemon"].includes(",")) {
									let vals = finaldata["Moves"]["Call"][r]["Pokemon"].split(",");
									...
									if (finaldata["Moves"]["Call"][r]["Pokemon"] == userPokPath.value) {
										check1 = true;
									}
								}
							}
...
function DMGUpdateSRC() {
	...
	for (let i = 0; i < bases.length; i++) {
		...
		let pokPath = pokBase.querySelector(":scope *[name='pokemon'] select");
		...
		if (pokPath.value != "") {
			...
			if (dataString["shiny"] != undefined && dataString["shiny"] == "true") {
				if (bases[i].parentElement.parentElement.getAttribute("name") == "user") {
					data_dir = [path_Pokemon_Battle_Shiny_Back_GIF,path_Pokemon_Battle_Shiny_Back_PNG,path.Pokemon.Battle.Shiny.Front_GIF,path.Pokemon.Battle.Shiny.Front_PNG]
				}
				else {
					data_dir = [path.Pokemon.Battle.Shiny.Front_GIF,path.Pokemon.Battle.Shiny.Front_PNG]
				}
			}
			else {
				if (bases[i].parentElement.parentElement.getAttribute("name") == "user") {
					data_dir = [path_Pokemon_Battle_Default_Back_GIF,path_Pokemon_Battle_Default_Back_PNG,path.Pokemon.Battle.Default.Front_GIF,path.Pokemon.Battle.Default.Front_PNG]
				}
				else {
					data_dir = [path.Pokemon.Battle.Default.Front_GIF,path.Pokemon.Battle.Default.Front_PNG]
				}
			}
			

			let data_file = ["^"+getPokemonPath(getPokemonInt(pokPath.value))]
			...
			pokImgPath.src = get_directory(true,data_file,data_dir,[],true)
		}
	}
...
function DMGSetChange(base) {
	...
	let pokPath = pokBase.querySelector(":scope *[name='pokemon'] select");
	let formPath = pokBase.querySelector(":scope *[name='pokemon'] > span input");
	...
	if (divBase.getAttribute("data-string") === null || divBase.getAttribute("data-string") === "") { return }

	let data = JSON.parse(divBase.getAttribute("data-string"));
	...
	if (data.Pokemon != undefined && getPokemonInt(data.Pokemon)) {
		...
		pokNamePath.innerText = data.Pokemon != null ? data.Pokemon : "";
		pokLvPath.innerText = data.Level != null ? "Lv. " + data.Level : "";
		pokPath.value = data.Pokemon != null ? data.Pokemon : "";
		...
		if (config.Held) {
			if (item != undefined) {
				...
				if (itemPath.value != undefined && itemPath.value != "") {
					...
					pokItemPath.src = get_directory({FirstMatch: true, File: [itoc], Path: [path.Item.Bag]})
				}
			}
		}
					...
					for (let v = 0; v < movesPath.length; v++) {
						if (moves[v] != undefined && moves[v] != "") {
							if (!usedm.includes(moves[v])) {
								...
								let val = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],moves[v])
								if (val != undefined) {
				...
				if (moves.length == pokMovesPath.length) {
					for (let v = 0; v < pokMovesPath.length; v++) {
						...
						if (val != undefined && !val.includes("#") && val != "") {
							let category = returnArrValue(finaldata["Moves"]["Category"],header.Move.Reference["Name"],header.Move.Category["Category"],val);
							let type = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],val)
...
function DMGSetPossible() {
	...
	if (moveRange == "Affects all Pokemon on the field") {
		for (let a = 0; a < all.length; a++) {
			tars.push(all[a])
		}
	}
...
function DMGPokSpecific(base) {
	...
	if (pokNameVal != undefined) {
		if (pokNameVal["pok"] != undefined) {
			int = getPokemonInt(pokNameVal["pok"]);
		}
	}
	if (int == undefined) {
		return;
	...
	let type1 = returnData(getPokemonInt(pokemonPath.value),"Type","")[0];
	let type2 = returnData(getPokemonInt(pokemonPath.value),"Type","")[1];
	...
	let pokForms = getPokemonForm(int);
	...
	if (Gender) {
		...
		if (getPokemonName(int).includes("Male")) {
			genders = ["♂"];
		}
		else if (getPokemonName(int).includes("Female")) {
			genders = ["♀"];
		}
		else {
...
function DMGSaveData(base) {
    ...
    let data_obj = dataStringToObj(datas);

    if (pokPath.value == undefined || pokPath.value == "") { return; }

    let int = getPokemonInt(pokPath.value);

    if (int == undefined) { return; }

	data_obj["Pokemon"] = getPokemonName(int,"Alt");
	data_obj["Level"] = lvlPath != undefined && lvlPath.value != undefined && lvlPath.value != "" ? lvlPath.value : "";
	data_obj["Ability"] = abilityPath != undefined && abilityPath.value != undefined && abilityPath.value != "" ? abilityPath.value : "";
	data_obj["Gender"] = genderPath != undefined && genderPath.value != undefined && genderPath.value != "" ? genderPath.value : "";
	data_obj["Item"] = itemPath != undefined && itemPath.value != undefined && itemPath.value != "" ? itemPath.value : "";
	data_obj["Friendship"] = friendshipPath != undefined && friendshipPath.value != undefined && friendshipPath.value != "" ? friendshipPath.value : "";
	data_obj["Nature"] = naturePath != undefined && naturePath.value != undefined && naturePath.value != "" ? naturePath.value : "";
	data_obj["Moves"] = movesPath != undefined ? Array.from(movesPath).map(select => select.value.includes("#") ? "" : select.value) : "";
	data_obj["IV"] = ivsPath != undefined ? Array.from(ivsPath).map(select => select.value === 0 ? "" : select.value).join(",") : "";
	data_obj["EV"] = evsPath != undefined ? Array.from(evsPath).map(select => select.value === 0 ? "" : select.value).join(",") : "";
	

    divBase.title = dataStringTitle(divBase.getAttribute("data-string"));
    divBase.setAttribute("data-string",JSON.stringify(data_obj))


	let tars = document.querySelectorAll("#contain > div#tool div#dmg div[name='battle'] span[name*='team'] > div[data-string*='Pokemon']");
	document.querySelector("#contain > div#tool div#dmg > div").setAttribute("data-count",tars.length);
...
function DMGMatchPosition() {
    ...
    let pokBases = document.querySelectorAll("#contain > div#tool div#dmg ol[name='pokemon'] > span[name] > ul[name]");
	let pokTopBases = document.querySelectorAll("#contain > div#tool div#dmg ol[name='pokemon'] > span[name]");
	...
	for (let p = 0; p < pokTopBases.length; p++) {
		...
		for (let r = 0; r < els.length; r++) {
			let x = r+1;
			let el = els[r].querySelector(":scope *[name='pokemon'] > *:first-child > *:first-child");
			el.innerText = "Pokemon #"+x;
		}
	}
...
function PKHeX_convert(str) {
	/*
	Aerodactyl (M)
	IVs: 0 HP / 0 Atk / 0 Def / 0 SpA / 0 SpD / 0 Spe
	EVs: 252 HP / 4 Atk / 2 Def / 252 Spe
	Ability: Pressure
	Level: 50
	Adamant Nature
	- Rock Slide
	...
	Magmar (M) @ Magmarizer
	IVs: 15 HP / 19 Atk / 26 Def / 24 SpA / 0 SpD / 2 Spe
	Ability: Flame Body
	Level: 30
	Gentle Nature
	- Smog
	...
	Magneton
	IVs: 23 HP / 0 Atk / 2 Def / 0 SpA / 2 SpD / 0 Spe
	EVs: 252 HP / 4 Atk / 2 Def / 252 Spe
	Ability: Sturdy
	Level: 53
	Shiny: Yes
	...
	*/
	...
	for (let i = 0; i < temp_arr.length; i++) {
		...
		const stats = Object.fromEntries(statsOrder.map(stat => [stat, 0])); // Default to 0
		
		console.log(temp_arr[i])

		// Extract Pokemon
		const pokemonMatch = temp_arr[i].match(/^([\w\.]+)\s\(/);
		data.Pokemon = pokemonMatch ? pokemonMatch[1] : null;

		// Extract Level
		const levelMatch = temp_arr[i].match(/Level:\s*(\d+)/);
		data.Level = levelMatch ? levelMatch[1] : null;

		// Extract Nature
		const natureMatch = temp_arr[i].match(/(\w+ Nature)/);
		data.Nature = natureMatch ? natureMatch[1].replaceAll(" Nature","") : null;

		// Extract Ability
		const abilityMatch = temp_arr[i].match(/Ability:\s*(.+)/);
		data.Ability = abilityMatch ? abilityMatch[1].trim() : null;

		// Extract IVs
		(temp_arr[i].match(/IVs:\s*(.*)/)?.[1]?.split(' / ').forEach(part => {
			const [value, stat] = part.split(' ');
			if (stat) stats[stat] = +value; // Update stat with number
		}));
		data.IVs = statsOrder.map(stat => stats[stat]).join(',');

		// Extract EVs
		(temp_arr[i].match(/EVs:\s*(.*)/)?.[1]?.split(' / ').forEach(part => {
			const [value, stat] = part.split(' ');
			if (stat) stats[stat] = +value; // Update stat with number
		}));
		data.EVs = statsOrder.map(stat => stats[stat]).join(',');

		// Extract Shiny status
		const shinyMatch = temp_arr[i].match(/Shiny:\s*(.+)/);
		data.Shiny = shinyMatch ? shinyMatch[1].trim() : null;

		// Extract Moves
		const movesMatch = temp_arr[i].match(/- (.+(?:\n- .+)*)/);
		data.Moves = movesMatch ? movesMatch[0].split('\n').map(move => move.trim().replace(/^- /, '')) : [];
...
function DMGSetDataString(base,str) {
	...
	if (str == undefined) {
        userInput = prompt("Enter Pokemon Data String:","");
	}
	else {
		userInput = str;
	}
	...
	if (userInput != null && userInput != "") {
		...
		const PKHeX_arr = PKHeX_convert(data)
		const data = PKHeX_arr.length > 0 ? PKHeX_arr : dataStringToObj(data)


		for (let d = 0; d < data.length; d++) {

			if (base[d] == undefined) { consoleText("Could not fit all Pokemon on the Field."); break; }
			...
			let pokBase = document.querySelector("#contain > div#tool div#dmg ol[name='pokemon'] span[name='"+baseteam+"'] > ul[name='"+baseid+"']");

			if (data[d].Pokemon === undefined) { consoleText("Pokemon Error."); break; }

			let dataInt = getPokemonInt(data[d].Pokemon);

			if (data[d].Pokemon != undefined && dataInt != undefined) {
				if (finaldata["Pokemon"]["Overview"][dataInt]["Active"] == "true") {
					divBase.setAttribute("data-string",JSON.stringify(data[d]));
...
function DMGPartyCreate(base,data) {
	if (data == undefined) {
		let ask = prompt("Enter Pokemon Data String");
		data = ask;
	}
	...
	if (data != null && data != "") {
		...
		for (let r = 0; r < data.length; r++) {
			...
			let pok = dataobj["pok"];
			let pokint = getPokemonInt(pok);
	
			if (pok != undefined && pokint != undefined) {
				if (finaldata["Pokemon"]["Overview"][pokint]["Active"] == "true") {
...
function DMGPartyActiveSet() {
	...
	vals.push(...(config.Natures != undefined ? ["na"] : []));
	vals.splice(1, 0, ...(config.Held == true ? ["it"] : []));
...
function buildDMG(preval) {
	...
	let optionsPokPath = document.querySelector("#contain > div#tool div#dmg > div ol[name='pokemon']");
	...
	for (let d = 0; d < battleTeams; d++) {
		...
		let pokCount = battleVariant["Pokemon"];
			...
			for (let q = 0; q < pokCount; q++) {
					...
					drop: function(e,ui) {
						...
						if (tar.tagName == "LI") {
							...
							if (tarString.includes("pok:") && !tarString.includes("pok:|") && !tarString.includes("pok:\n")) {
								if (finaldata["Pokemon"]["Overview"][getPokemonInt(dataStringToObj(tarString)["pok"])]["Active"] == "true") {
				...
				*/
				...
				for (let m = 0; m < 4; m++) {
					...
					//contentMove.addEventListener("click",function() {let val = this.firstChild.innerText;let tar = document.querySelector("#contain > div#tool div#dmg div[name='menu'] *[name='move'] select");let tarTemp = document.querySelector("#contain > div#tool div#dmg div[name='menu'] > div[name='move'] > span select > option[value='"+val+"']"); if (val != "") {tar.parentElement.style.color = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],val)+")"; if (tarTemp != undefined) {tar.value = val;} DMGSetInfo();DMGCalcStart();let movd = formatMoveData(val);movd = undDel(movd,"");tar.title = movd;}});1
		...
		if ("Options") {
            ...
            for (let q = 0; q < pokCount; q++) {
				...
				optionsPokOption.innerText = "Pokemon #"+y;
			...
			for (let q = 0; q < pokCount; q++) {
                ...
                if ("Pokemon") {
					...
					pokSelect.setAttribute("name","pokemon");
                    pokTitleText.innerText = "Pokemon #"+y;
                    pok.setAttribute("name","pokemon");
                    ...
                    for (let e = 0; e < finaldata["Pokemon"]["Overview"].length; e++) {
                        if (finaldata["Pokemon"]["Overview"][e]["Active"] == "true") {
                            poks.push(getPokemonName(e))
                        }
                    }

                    for (let e = 0; e < poks.length; e++) {
						let pokint = getPokemonInt(poks[e]);
                        let pokOption = document.createElement("option");
                        pokSelect.appendChild(pokOption);

                        if (pokint != undefined) {
							let pokName = getPokemonName(pokint,"Alt");
							let pokTitle = getPokemonName(pokint);
                            ...
                            pokOption.setAttribute("data-variant",finaldata["Pokemon"]["Overview"][pokint]["Variant"]);
                        }
                ...
                if ("Nature") {
                    if (config.Natures != undefined) {
                        ...
                        let naturesTemp = [...(config.Natures)];
                ...
                if ("Move") {
                    ...
                    for (let m = 0; m < 4; m++) {
                        ...
                        moveSelect.addEventListener("change",function() {let x = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],this.value); if (x == undefined) {this.style.removeProperty("color");this.style.removeProperty("background");} else {this.style.color = "var(--type"+x+")";}})
                        ...
                        for (let t = 0; t < finaldata["Moves"]["Overview"].length; t++) {
                            if (finaldata["Moves"]["Overview"][t]["Active"] == "true") {
                                if (finaldata["Moves"]["Group"][t]["Group"] != "Z-Move" && finaldata["Moves"]["Group"][t]["Group"] != "Max Move" && finaldata["Moves"]["Group"][t]["Group"] != "G-Max Move") {
                                    tempMoves.push(finaldata["Moves"]["Overview"][t][header.Move.Reference["Name"]])
                                }
                            }
                        }
                        ...
                        for (let n = 0; n < movesTemp.length; n++) {
                            ...
                            if (n != 0) {
                                moveOption.style.background = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],movesTemp[n])+")";
                            }
                        }
                ...
                if ("Export") {
                    ...
                    let exportOptions = ["Import Pokemon","Copy Data String","Add Copy to Party","Add Copy to Box","Change Evolution","Change Form"];
                ...
                for (let c = 0; c < battleCondition.length; c++) {
                    if (battleCondition[c]["Affect"] == "Pokemon") {
                        var check = get_applicable(battleCondition[c]["Game"]);
                        ...
                        if (check) {
                            ...
                            if (appender != undefined) {
                                ...
                                if (battleCondition[c]["Title"] != undefined) {
                                    ...
                                    let val = returnArrValue(finaldata["Moves"]["Description"],header.Move.Reference["Name"],header.Move.Description["Description"],battleCondition[c]["Name"])
                                    if (val != undefined) {
            ...
            for (let q = 0; q < pokCount; q++) {
                ...
                for (let e = 0; e < statsCol.length; e++) {
                    ...
                    if (statsTitle == "IV") {
                        if (get_applicable("Red,Blue,Yellow,Gold,Silver,Crystal")) {
                            statsMax = 15;
                        }
                    }
                    if (statsTitle == "EV") {
                        if (get_applicable("Lets Go Pikachu,Lets Go Eevee")) {
                    ...
                    for (let s = 0; s < statsTemp.length; s++) {
						...
						statsWrapInput.addEventListener("input",function() {let x = findUpTag(this,"UL");let base2 = document.querySelector("#contain > div#tool div#dmg div[name='options'] ol[name='pokemon'] > span[name='"+x.parentElement.getAttribute("name")+"'] > ul[name='"+x.getAttribute("name")+"']");DMGCalcPokStats(base2)});
						statsWrapInput.addEventListener("input",function() {let x = findUpTag(this,"UL");let base2 = document.querySelector("#contain > div#tool div#dmg div[name='options'] ol[name='pokemon'] > span[name='"+x.parentElement.getAttribute("name")+"'] > ul[name='"+x.getAttribute("name")+"']");DMGSaveData(base2)});
						//statsWrapInput.addEventListener("input",function() {let x = findUpTag(this,"UL");let base2 = document.querySelector("#contain > div#tool div#dmg div[name='options'] ol[name='pokemon'] > span[name='"+x.parentElement.getAttribute("name")+"'] > ul[name='"+x.getAttribute("name")+"']");DMGCalcStart(base2)});
					...
					if (check) {
						...
						if (appender != undefined) {
							...
							if (battleCondition[c]["Group"] == "Badge" || battleCondition[c]["Name"] == "Tailwind") {
								conditionInput.addEventListener("change",function() {let base = findUpTagAtt(this,"SPAN","name"); let team = base.getAttribute("name"); let els = document.querySelectorAll("#contain > div#tool div#dmg > div ol[name='pokemon'] > span[name='"+team+"'] > ul"); for (let i = 0; i < els.length; i++) {DMGCalcPokStats(els[i])}});
							}
				...
				if (appender != undefined) {
					...
					if (appender != undefined) {
						...
						if (battleCondition[c]["Group"] == "Weather") {
							...
							img.src = get_directory(true,battleCondition[c]["Name"],[path.Weather.Icon.PNG,path.Weather.Icon.GIF])
						...
						if (battleCondition[c]["Group"] == "Terrain") {
							...
							let img = document.createElement("img");
							img.src = get_directory({FirstMatch: true, File: [terrtype],Path: [path.Type.Icon], Game: ["GO"]})
							img.title = battleCondition[c]["Name"];
...
function DMGExportChange() {
    ...
    let pokPath = pokBase.querySelector(":scope li[name='pokemon'] select");
    let int = undefined;
    if (pokPath.value != undefined && pokPath.value != "") {
        int = getPokemonInt(pokPath.value);
    }


    if (val == "Import Pokemon") {
        DMGSetDataString(divBase);
    }

    if (int != undefined) {
        let evoFamily = getEvolutionFamily(int).map(function(v) {return v["Pokemon"];});
        let pokForm = getPokemonForm(int);
        ...
        if (val == "Add Copy to Party") {
            let slot = document.querySelectorAll('#pokemon > aside[name="team"] section div[name="empty"]');
            if (slot.length > 0) {
        ...
        if (val == "Change Evolution") {
            evoFamily = evoFamily.filter(function(v) {
                return v != finaldata["Pokemon"]["Overview"][int]["Pokemon"];
            })
            evoFamily = evoFamily.filter(function(v) {
                return v != finaldata["Pokemon"]["Form"][int][header.Pokemon.Form["Form"]];
            })
