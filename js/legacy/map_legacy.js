let create_map = function() {
	...
	mapSectionContentMapImg.src = get_directory({FirstMatch: true, File: ["Map"], Path: [path.Region.Map]})

	mapSectionContentMapImg.setAttribute("usemap","#"+regions+"-map");
	mapOuter.appendChild(mapSectionContent);
	mapSectionContent.appendChild(mapSectionContentMapOuter);

	mapSectionContentMap.setAttribute("name",regions+"-map");
	mapSectionContentMap.setAttribute("id",regions+"-map");
    ...
    mapSectionHeaderGameImage.src = get_directory({FirstMatch: true, File: ["Title"], Path: [path.Game.Title]})
	...
	mapSectionSidebarDescriptionPok.setAttribute("name", "pokemon");
	...
	let location_array = []
	for (let q = 0; q < finaldata["Locations"]["Overview"].length; q++) {
		if (get_applicable(finaldata["Locations"]["Overview"][q]["Game"])) {
			location_array.push(finaldata["Locations"]["Overview"][q])
			location_array[location_array.length-1]["Index"] = q;
		}
	}
	location_array = location_array.sort((a, b) => new Intl.Collator([], {numeric: true}).compare(a["Location"], b["Location"]));


	for (let q = 0; q < location_array.length; q++) {
		...
		mapSectionListOptionsInput.value = location_array[q]["Index"];
		mapSectionListOptionsLabel.setAttribute("for", "map-options-"+q);
		mapSectionListOptionsLabel.setAttribute("type","medium");
		mapSectionListOptionsLabel.setAttribute("data-name", location_array[q]["Location"].toLowerCase());
		mapSectionListOptionsLabel.setAttribute("data-title", location_array[q]["Location"].toLowerCase());
		let poi = [];
		for (let u = 0; u < finaldata["Locations"]["Point of Interest"].length; u++) {
			if (get_applicable(finaldata["Locations"]["Point of Interest"][u]["Game"])) {
				if (finaldata["Locations"]["Point of Interest"][u]["Location"] == location_array[q]["Location"]) {
		...
		for (let u = 0; u < finaldata["Locations"]["Navigation"].length; u++) {
			if (finaldata["Locations"]["Navigation"][u]["Location"] == location_array[q]["Location"]) {
		...
		mapSectionListOptionsText.innerText = location_array[q]["Location"];
	...
	let location_searches = document.querySelectorAll("#contain > div#map > section[name='list'] ol > label");
    searchMapAttributes = [];
    for (q = 0; q < location_searches.length; q++) {
        for (u = 0; u < location_searches[q].getAttributeNames().length; u++) {
            if (location_searches[q].getAttributeNames()[u].includes("data-search")) {
                if (!searchMapAttributes.includes(location_searches[q].getAttributeNames()[u])) {
                    searchMapAttributes.push(location_searches[q].getAttributeNames()[u]);
                }
            }
        }
    }
	...
	function mapOptionsSelector(i) {
		...
		let location = finaldata["Locations"]["Overview"][i]["Location"];
		let trainers = getArrDataIndexed(finaldata["Location Trainers"]["Trainers"],"Location",location);
		let shops = getArrDataIndexed(finaldata["Location Pokemon"]["Shop"],"Location",location).concat(getArrDataIndexed(finaldata["Location Items"]["Shop"],"Location",location));
		let items = getArrDataIndexed(finaldata["Location Items"]["Items"],"Location",location);
		let poks = getArrDataIndexed(finaldata["Location Pokemon"]["Pokemon"],"Location",location);
		...
		let mapDescriptionTitles = ["Overview","Pokemon", "Items", "Shop", "Trainers", "Move Tutor"];

		if (poks.length == 0) {
			mapDescriptionTitles = mapDescriptionTitles.filter((v) => !v.includes("Pokemon"));
		}
		if (items.length == 0) {
		...
		for (let q = 0; q < finaldata["Locations"]["Description"].length; q++) {
			if (get_applicable(finaldata["Locations"]["Description"][q]["Game"])) {
		...
		for (let q = 0; q < finaldata["Locations"]["Point of Interest"].length; q++) {
			if (get_applicable(finaldata["Locations"]["Point of Interest"][q]["Game"])) {
		...
		let loadImages = get_directory(false,[location],[path.Location.Load]);
		loadImages = loadImages.filter(x => x != null && x != undefined && x != '')
	
		let overviewImages = get_directory(false,[location],[path.Location.Overview]);
		...
		for (let q = 0; q < shopArea.length; q++) {
			...
			for (let u = 0; u < shops.length; u++) {
				if (shops[u]["Sort"] == shopArea[q]) {
					...
					else if (shops[u]["Pokemon"] != undefined) {
						main = shops[u]["Pokemon"];
						type = "pokemon";
					}
					...
					if (type == "item") {
						...
						mapSectionSidebarDescriptionShopIconOuter.setAttribute("value",getPokemonInt(main))
					...
					for (let y = 0; y < quantity; y++) {
						...
						if (type == "item") {
							...
							if (itoc != undefined) {
								mapSectionSidebarDescriptionShopIcon.src = get_directory({FirstMatch: true, File: [itoc], Path: [path.Item.Bag]});
							}
							mapSectionSidebarDescriptionShopIcon.setAttribute("onerror", "this.style.display='none';");
						}
						else if (type == "pokemon") {
							mapSectionSidebarDescriptionShopIcon.src = get_directory({FirstMatch: true, File: [getPokemonPath(getPokemonInt(main))], Path: [path.Pokemon.Box.Default.PNG]});
						}
					...
					for (let r = 0; r < currency.length; r++) {
						...
						let icon;
						if (ic != undefined) {
							icon = get_directory({FirstMatch: true, File: [ic], Path: [path.Currency.Icon]});
						}
			...
			for (let u = 0; u < items.length; u++) {
				if (items[u]["Sort"] == itemArea[q]) {
					...
					for (let y = 0; y < quantity; y++) {
						...
						if (itoc != undefined) {
							mapSectionSidebarDescriptionItemIcon.src = get_directory({FirstMatch: true, File: [itoc], Path: [path.Item.Bag]})
						}
						...
						if (items[u]["Field"].includes("/")) {
							for (let y = 0; y < items[u]["Field"].split("/").length; y++) {
								...
								if (itemIcon != undefined) {
									...
									mapSectionSidebarDescriptionItemFieldImage.src = get_directory({FirstMatch: true, File: [itemIcon], Path: [path.Item.Bag]})
		...
		for (let q = 0; q < pokArea.length; q++) {
			...
			for (let u = 0; u < poks.length; u++) {
				if (poks[u]["Sort"] == pokArea[q]) {
					...
					mapSectionSidebarDescriptionPokLiInput.setAttribute("id","location-pokemon-"+poks[u]["Index"]);
					mapSectionSidebarDescriptionPokLiInput.setAttribute("name","location-pokemon-"+poks[u]["Index"]);
					...
					mapSectionSidebarDescriptionPokIcon.src = get_directory({FirstMatch: true, File: [getPokemonPath(getPokemonInt(poks[u]["Pokemon"]))], Path: [path.Pokemon.Box.Default.PNG]});
					mapSectionSidebarDescriptionPokWrap.setAttribute("value",getPokemonInt(poks[u]["Pokemon"]));
					mapSectionSidebarDescriptionPokNameText.innerText = poks[u]["Pokemon"];
					...
					if (config.Allies) {
						...
						if (poks[u]["Allies"] != undefined) {
							...
							for (let r = 0; r < allies.length; r++) {
								...
								mapSectionSidebarDescriptionPokAlliesIMG.src = get_directory({FirstMatch: true, File: [getPokemonPath(getPokemonInt(allies[r]))], Path: [path.Pokemon.Box.Default.PNG]});
								mapSectionSidebarDescriptionPokAlliesIMG.title = allies[r];
								mapSectionSidebarDescriptionPokAlliesIMG.setAttribute("value",getPokemonInt(allies[r]));
					...
					let val2 = [path.Object.Overworld.Tile];
					let val3 = getGames("Generation");
					let val4 = poks[u]["Tile"];

					if (poks[u]["Encounter"] == "Static") {
						pok_int = getPokemonInt(poks[u]["Pokemon"]);
						val1 = ["^"+getPokemonPath(pok_int),"^"+getPokemonPath(pok_int)+"_Male","^"+getPokemonPath(pok_int)+"_Female"]
						val2 = [path_Pokemon_Overworld_Default_Front,path_Pokemon_Overworld_Shiny_Front]
						val3 = [GameName];
						val4 = poks[u]["Pokemon"];
					}

					mapSectionSidebarDescriptionPokTypeEncounterImg.src = get_directory(true,val1,val2,val3);
		...
		for (let q = 0; q < tutorArea.length; q++) {
			...
			for (let u = 0; u < tutors.length; u++) {
				if (tutors[u]["Area"] == tutorArea[q]) {
					...
					mapSectionSidebarDescriptionTutorMoveText.style.color = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],tutors[u]["Move"])+")";
					...
					if (tutors[u]["Pokemon"] != undefined || tutors[u]["Requirement"] != undefined || tutors[u]["Cost"] != undefined || tutors[u]["Rate"] != undefined || tutors[u]["Time"]) {
						...
						if (tutors[u]["Pokemon"] != undefined) {
							...
							mapSectionSidebarDescriptionTutorPokHeader.innerText = "Pokemon:"
							mapSectionSidebarDescriptionTutorPokText.innerHTML = "<b type='invert' onclick='modalData()' function='modalData' value='"+getPokemonInt(tutors[u]["Pokemon"])+"'>"+tutors[u]["Pokemon"]+"</b>";
						...
						if (tutors[u]["Cost"] != undefined) {
							...
							if (tutor_cost.length == tutor_currency.length) {
								for (let c = 0; c < tutor_cost.length; c++) {
									...
									let currency_icon = get_directory({FirstMatch: true, File: [tutor_currency[c]], Path: [path.Item.Bag]});
		...
		for (let q = 0; q < finaldata["Locations"]["Slogan"].length; q++) {
			if (get_applicable(finaldata["Locations"]["Slogan"][q]["Game"])) {
		...
		for (let q = 0; q < finaldata["Locations"]["Navigation"].length; q++) {
			if (get_applicable(finaldata["Locations"]["Navigation"][q]["Game"])) {
				if (finaldata["Locations"]["Navigation"][q]["Location"] == location) {
					for (let u = 0; u < finaldata["Locations"]["Navigation"][q]["Navigation"].split(",").length; u++) {
						...
						if (finaldata["Locations"]["Navigation"][q]["Navigation"].split(",")[u] == "Cut" || finaldata["Locations"]["Navigation"][q]["Navigation"].split(",")[u] == "Strength") {
							mapSectionContentNavigationInnerImg.src = get_directory({FirstMatch: true, File: [`HM Normal`], Path: [path.Item.Bag]})
							...
							mapSectionContentNavigationInnerImg.src = get_directory({FirstMatch: true, File: [`HM Water`], Path: [path.Item.Bag]});
							...
							mapSectionContentNavigationInnerImg.src = get_directory({FirstMatch: true, File: [`HM Fighting`], Path: [path.Item.Bag]});
						} else if (finaldata["Locations"]["Navigation"][q]["Navigation"].split(",")[u] == "Defog") {
							mapSectionContentNavigationInnerImg.src = get_directory({FirstMatch: true, File: [`HM Flying`], Path: [path.Item.Bag]});
						} else {
							mapSectionContentNavigationInnerImg.src = get_directory({FirstMatch: true, File: [finaldata["Locations"]["Navigation"][q]["Navigation"].split(",")[u]], Path: [path.Item.Bag]});
						}
		...
		for (let q = 0; q < finaldata["Locations"]["Connecting"].length; q++) {
			if (get_applicable(finaldata["Locations"]["Connecting"][q]["Game"])) {
	...
	mapSectionSidebarDescriptionPok.addEventListener("scroll",function() {updateTitleHeader("pokemon")});
	...
	for (let i = 0; i < finaldata["Locations"]["Connecting"].length; i++) {
		if (get_applicable(finaldata["Locations"]["Connecting"][i]["Game"])) {
					...
					if (val.includes(",")) {
						...
						for (let u = 0; u < val.length; u++) {
							for (let r = 0; r < finaldata["Locations"]["Connecting"].length; r++) {
								if (get_applicable(finaldata["Locations"]["Connecting"][r]["Game"])) {
...
function getLocationTrainers(l) {
	...
	for (let i = 0; i < finaldata["Location Trainers"]["Trainers"].length; i++) {
		if (finaldata["Location Trainers"]["Trainers"][i]["Location"] == l && get_applicable(finaldata["Location Trainers"]["Trainers"][i]["Game"])) {
...
function updateTrainer(trainers,condition) {
	...
	currentImg.src = get_directory(true,trainerImage,[path_Character_Battle_Front,path_Character_VS])
	...
	if (trainerPrevious != undefined) {
		...
		previousImg.src = get_directory(true,previmg,[path_Character_Battle_Front,path_Character_VS])
	...
	if (trainerNext != undefined) {
		...
		nextImg.src = get_directory(true,nextimg,[path_Character_Battle_Front,path_Character_VS])
	...
	if (trainerReward != undefined) {
		...
		let reward_img = get_directory({FirstMatch: true, File: [getItemIcon(trainerReward)], Path: [path.Item.Bag,path.Currency.Icon]});
	...
	if (trainerItem != undefined) {
		...
		for (let u = 0; u < itcount; u++) {
			...
			itemsImg.src =  get_directory({FirstMatch: true, File: [getItemIcon(trainerItem)], Path: [path.Item.Bag]});
			if (itcount != 1) {
	...
	let groups = [];
	let datas = [];
	if (trainers[q]["Pokemon"].includes("\n")) {
		datas = trainers[q]["Pokemon"].split("\n");
	}
	else {
		datas = [trainers[q]["Pokemon"]]
	}
	...
	for (let u = 0; u < datas.length; u++) {
		...
		if (grp != undefined) {
			...
			function grpFigFunction() {
				...
				if (val == "Copy Data Strings") {
					...
					let ask = confirm("Do you want to replace existing Pokemon in the Party?");
					if (ask) {
		...
		if (u == 0) {
			...
			if (trainerCount != undefined) {
				for (let t = 0; t < trainerCount; t++) {
					...
					span.firstChild.classList.add("pokeball");
				}
			}
			else {
				for (let t = 0; t < 6; t++) {
		...
		if (nature != undefined || ability != undefined || level != undefined || gender != undefined) {
			...
			if (ability != undefined) {
				...
				abilityWrap.title = getAbilityPosition(getPokemonInt(pok),ability)+" Ability";
				if (desc.length > 0) {
					abilityWrap.title += "\n"+desc[0]["Description"];
				}
				else {
		...
		pokCenter.setAttribute("name", "pokemon");
		liMain.appendChild(pokCenter);
		pokCenter.appendChild(pokCenterWrap);

		if (item != undefined) {
			let heldImg = document.createElement("img");
			heldImg.src = get_directory({FirstMatch: true, File: [getItemIcon(item)], Path: [path.Item.Bag]});
		...
		pokImg.src = get_directory({FirstMatch: true, File: ["^"+getPokemonPath(getPokemonInt(pok)),"^"+getPokemonPath(getPokemonInt(pok))+"_Male","^"+getPokemonPath(getPokemonInt(pok))+"_Female"], Path: [path.Pokemon.Battle.Default.Front_PNG,path.Pokemon.Battle.Default.Front_GIF]});
		pokImg.setAttribute("title", pok);
		pokCenterWrap.setAttribute("value",getPokemonInt(pok));
		...
		if (move != undefined || iv != undefined || ev != undefined) {
			...
			if (move != undefined) {
				...
				for (let y = 0; y < moves.length; y++) {
					...
					pokRightMovesWrap.style.color = "var(--type"+returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],moves[y])+")";
					...
					if (moves[y] != "") {
						if (returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],moves[y]) == undefined) {
							console.warn(moves[y]+" needs formatting?");
						}
						
						let moveset = returnMoveSet(getPokemonInt(pok),"onlymoves,noduplicate");
