let create_nav = function() {

	const navOptions = ["Pokemon","Moves","Items","Map","Tools"];
	navOptions.splice(2, 0, ...(data.Abilities !== undefined ? ["Abilities"] : []));
...
function createEditor(data) {
	let options = [{"name":"Pokemon","data":"pok","type":"Select","game":"All"},{"name":"Nickname","data":"ni","type":"Text","game":"All"},{"name":"Level","data":"lv","type":"Number","game":"All"},{"name":"Ability","data":"ab","type":"Select","game":"3+"},{"name":"Nature","data":"na","type":"Select","game":"3+"},{"name":"Item","data":"it","type":"Select","game":"2+"},{"name":"Gender","data":"ge","type":"Select","game":"2+"},{"name":"Friendship","data":"fr","type":"Number","game":"2+"},{"name":"Move","data":"mo","type":"Select","game":"All"},{"name":"Individual Value","data":"iv","type":"Number","game":"All"},{"name":"Effort Value","data":"ev","type":"Number","game":"All"},{"name":"Shiny","data":"shiny","type":"Boolean","game":"2+"},{"name":"Condition","data":"cond","type":"Number","game":"3+"},{"name":"Marking","data":"mark","type":"Select","game":"3+"},{"name":"Met Location","data":"metlo","type":"Select","game":"All"},{"name":"Met Level","data":"metlv","type":"Number","game":"All"},{"name":"Met Date","data":"metda","type":"Date","game":"All"},{"name":"Ball","data":"ball","type":"Select","game":"All"},{"name":"Pokemon ID","data":"pid","type":"Text","game":"All"},{"name":"Original Trainer","data":"ot","type":"Text","game":"All"},{"name":"Original Trainer Gender","data":"otge","type":"Select","game":"All"},{"name":"Trainer ID","data":"tid","type":"Number","game":"All"},{"name":"Secret ID","data":"sid","type":"Number","game":"All"},{"name":"Version","data":"ver","type":"Select","game":"All"},{"name":"Language","data":"lang","type":"Select","game":"All"}];
	...
	for (let d = 0; d < data.length; d++) {
		...
		for (let o = 0; o < options.length; o++) {
			if (get_applicable(options[o]["game"])) {
				...
				for (let i = 0; i < int.length; i++) {
					...
					if (options[o]["type"] == "Text") {
						...
						if (options[o]["name"] == "Level" || options[o]["name"] == "Met Level") {
							...
							else if (config.Generation >= 7 && config.Generation <= 8) {
								selector.min = 0;
								selector.max = 999999;
						...
						if (options[o]["name"] == "Pokemon") {
							...
							opt = ["Poke Ball","Great Ball","Ultra Ball","Safari Ball","Smoke Ball","Heavy Ball","Level Ball","Lure Ball","Fast Ball","Light Ball","Friend Ball","Moon Ball","Love Ball","Park Ball","Net Ball","Dive Ball","Nest Ball","Repeat Ball","Timer Ball","Luxury Ball","Premier Ball","Dusk Ball","Heal Ball","Quick Ball","Cherish Ball","Sport Ball","Dream Ball","Beast Ball","Master Ball"];
						...
						else if (options[o]["name"] != "Pokemon") {
							opt.unshift("")
						}
	...
	function buttonFunc() {
		...
		if (tar.classList.contains("change")) {
			let base1 = document.querySelectorAll("#pokemon > aside section[name='party'] > div:not([data-string=''])");
...
function editorUpdateStyles() {
	...
	for (let i = 0; i < bases.length; i++) {
		let pok = bases[i].querySelector(":scope *[name='Pokemon'] select");
		let pokimg = bases[i].querySelector(":scope *[name='Pokemon'] img");
		...
		if (bases[i].parentElement.tagName == "TBODY") {
			for (let q = 0; q < move.length; q++) {
				if (move[q].value != "") {
					let moveType = returnArrValue(finaldata["Moves"]["Type"],header.Move.Reference["Name"],header.Move.Type["Type"],move[q].value);
			...
			let path = [path.Pokemon.Box.Default.PNG,path_Pokemon_Box_Default_GIF];
			if (shiny != undefined && shiny.checked) {
				path = [path_Pokemon_Box_Shiny_PNG,path_Pokemon_Box_Shiny_GIF,path.Pokemon.Box.Default.PNG,path_Pokemon_Box_Default_GIF]
			}
			let games = [...(AllGames)];
			games.unshift(GameName)
			
			let med = get_directory(true,[getPokemonPath(getPokemonInt(pok.value))],path)
...
function editorPopulateSpecific() {
	...
	for (let i = 0; i < row.length; i++) {
		let pokemon = row[i].querySelector(":scope td[name='Pokemon'] select");
		...
		let int = getPokemonInt(pokemon.value);
		if (int != undefined && pokemon != undefined && pokemon.value != "") {
...
function getPossible(i,type) {
	...
	if (type == "Item") {
        let items = [];
		let arr = finaldata["Items"]["Overview"].map(el => el["Pocket"] == "Berries" ? {...el, ["Pocket"]: "a"} : el).map(el => el["Pocket"] == "Items" || el["Pocket"] == "Other Items"  ? {...el, ["Pocket"]: "b"} : el).map(el => el["Pocket"] != "a" && el["Pocket"] != "b" ? {...el, ["Pocket"]: "c"} : el);
